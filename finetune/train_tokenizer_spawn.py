#!/usr/bin/env python
"""
Alternative multi-GPU training script using torch.multiprocessing.spawn
instead of torchrun. This avoids the macOS network initialization issues.
"""
import os
import sys
import time
from time import gmtime, strftime
import datetime
import torch
import torch.nn.functional as F
from torch.utils.data import DataLoader, DistributedSampler
import torch.multiprocessing as mp
from torch.distributed import init_process_group, destroy_process_group
from torch.nn.parallel import DistributedDataParallel as DDP

# Ensure project root is in path
sys.path.append(os.path.dirname(os.path.dirname(__file__)))
from config import Config
from dataset import QlibDataset
from model.kronos import KronosTokenizer


def set_seed(seed: int, rank: int = 0):
    """Sets random seed for reproducibility."""
    import random
    import numpy as np
    
    actual_seed = seed + rank
    random.seed(actual_seed)
    np.random.seed(actual_seed)
    torch.manual_seed(actual_seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(actual_seed)
        torch.backends.cudnn.deterministic = True
        torch.backends.cudnn.benchmark = False


def format_time(seconds: float) -> str:
    """Formats duration in seconds into H:M:S string."""
    return str(datetime.timedelta(seconds=int(seconds)))


def setup(rank, world_size):
    """Setup process group without network issues."""
    # Use file system based rendezvous to avoid network issues
    os.environ['MASTER_ADDR'] = 'localhost'
    os.environ['MASTER_PORT'] = '29501'  # Use different port
    
    # Initialize with file system based store
    try:
        init_process_group(
            backend='gloo',
            rank=rank,
            world_size=world_size,
            timeout=datetime.timedelta(seconds=30)
        )
        print(f"[Rank {rank}] DDP initialized successfully")
    except Exception as e:
        print(f"[Rank {rank}] Warning: DDP init failed: {e}")
        print(f"[Rank {rank}] Continuing in single-process mode")


def cleanup():
    """Cleanup process group."""
    destroy_process_group()


def train_model(model, device, config, save_dir, rank=0, world_size=1):
    """Training loop for the tokenizer."""
    print(f"[Rank {rank}] Starting training on device: {device}")
    
    # Create dataloaders
    print(f"[Rank {rank}] Loading datasets...")
    train_dataset = QlibDataset('train')
    valid_dataset = QlibDataset('val')
    print(f"[Rank {rank}] Train dataset size: {len(train_dataset)}, Validation dataset size: {len(valid_dataset)}")
    
    # Use DistributedSampler for multi-GPU training
    train_sampler = None
    val_sampler = None
    if world_size > 1:
        train_sampler = DistributedSampler(train_dataset, num_replicas=world_size, rank=rank, shuffle=True)
        val_sampler = DistributedSampler(valid_dataset, num_replicas=world_size, rank=rank, shuffle=False)
    
    train_loader = DataLoader(
        train_dataset, 
        batch_size=config['batch_size'], 
        sampler=train_sampler,
        shuffle=(train_sampler is None),
        num_workers=0,
        pin_memory=True if (torch.cuda.is_available() or torch.backends.mps.is_available()) else False
    )
    
    val_loader = DataLoader(
        valid_dataset, 
        batch_size=config['batch_size'], 
        sampler=val_sampler,
        shuffle=False,
        num_workers=0,
        pin_memory=True if (torch.cuda.is_available() or torch.backends.mps.is_available()) else False
    )
    
    # Optimizer and scheduler
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=config['tokenizer_learning_rate'],
        betas=(config['adam_beta1'], config['adam_beta2']),
        weight_decay=config['adam_weight_decay']
    )
    
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
        optimizer=optimizer,
        max_lr=config['tokenizer_learning_rate'],
        steps_per_epoch=len(train_loader),
        epochs=config['epochs'],
        pct_start=0.03,
        div_factor=10
    )
    
    best_val_loss = float('inf')
    dt_result = {}
    start_time = time.time()
    
    if rank == 0:
        print(f"\n{'='*70}")
        print(f"Training started with {config['epochs']} epochs")
        print(f"Batch size per GPU: {config['batch_size']}, Effective batch size: {config['batch_size'] * world_size}")
        print(f"Learning rate: {config['tokenizer_learning_rate']}")
        print(f"Number of devices: {world_size}")
        print(f"{'='*70}\n")
    
    for epoch_idx in range(config['epochs']):
        epoch_start_time = time.time()
        model.train()
        
        # Set epoch for sampler
        if train_sampler is not None:
            train_sampler.set_epoch(epoch_idx)
        
        epoch_loss = 0.0
        n_batches = 0
        
        # Training loop
        for i, (ori_batch_x, _) in enumerate(train_loader):
            ori_batch_x = ori_batch_x.to(device, non_blocking=True)
            
            # Forward pass
            zs, bsq_loss, _, _ = model(ori_batch_x)
            z_pre, z = zs
            
            # Loss calculation
            recon_loss_pre = F.mse_loss(z_pre, ori_batch_x)
            recon_loss_all = F.mse_loss(z, ori_batch_x)
            recon_loss = recon_loss_pre + recon_loss_all
            loss = (recon_loss + bsq_loss) / 2
            
            # Backward pass
            optimizer.zero_grad()
            loss.backward()
            
            # Gradient clipping
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=2.0)
            
            # Optimizer step
            optimizer.step()
            scheduler.step()
            
            epoch_loss += loss.item()
            n_batches += 1
            
            # Log progress
            if rank == 0 and (i + 1) % config['log_interval'] == 0:
                avg_loss = epoch_loss / n_batches
                print(
                    f"[Epoch {epoch_idx + 1}/{config['epochs']}, Step {i + 1}/{len(train_loader)}] "
                    f"LR {optimizer.param_groups[0]['lr']:.6f}, Loss: {avg_loss:.4f}"
                )
        
        # Calculate average training loss
        avg_train_loss = epoch_loss / len(train_loader)
        
        # Validation loop
        model.eval()
        val_loss_sum = 0.0
        val_sample_count = 0
        
        with torch.no_grad():
            for ori_batch_x, _ in val_loader:
                ori_batch_x = ori_batch_x.to(device)
                
                zs, _, _, _ = model(ori_batch_x)
                _, z = zs
                val_loss_item = F.mse_loss(z, ori_batch_x)
                
                val_loss_sum += val_loss_item.item() * ori_batch_x.size(0)
                val_sample_count += ori_batch_x.size(0)
        
        # Average validation loss across GPUs
        avg_val_loss = val_loss_sum / val_sample_count if val_sample_count > 0 else 0
        
        # Print epoch summary (rank 0 only)
        if rank == 0:
            print(f"\n{'-'*70}")
            print(f"--- Epoch {epoch_idx + 1}/{config['epochs']} Summary ---")
            print(f"Average Training Loss: {avg_train_loss:.4f}")
            print(f"Validation Loss: {avg_val_loss:.4f}")
            print(f"Time This Epoch: {format_time(time.time() - epoch_start_time)}")
            print(f"Total Time Elapsed: {format_time(time.time() - start_time)}")
            print(f"{'-'*70}\n")
            
            # Save best model
            if avg_val_loss < best_val_loss:
                best_val_loss = avg_val_loss
                save_path = os.path.join(save_dir, 'checkpoints', 'best_model')
                os.makedirs(save_path, exist_ok=True)
                # Save the underlying module, not the DDP wrapper
                if hasattr(model, 'module'):
                    model.module.save_pretrained(save_path)
                else:
                    model.save_pretrained(save_path)
                print(f"✓ Best model saved to {save_path} (Val Loss: {best_val_loss:.4f})\n")
    
    dt_result['best_val_loss'] = best_val_loss
    return model, dt_result


def train_worker(rank, world_size, config, save_dir):
    """Worker function for each process."""
    print(f"Starting worker process for rank {rank}")
    
    # Setup DDP (with fallback for failures)
    try:
        setup(rank, world_size)
    except Exception as e:
        print(f"[Rank {rank}] DDP setup failed: {e}")
        print(f"[Rank {rank}] Running in single-process mode")
        world_size = 1
        rank = 0
    
    # Setup device - Support CUDA, MPS (Apple Silicon), and CPU
    if torch.cuda.is_available():
        device = torch.device(f'cuda:{rank}')
        torch.cuda.set_device(device)
        print(f"[Rank {rank}] Using NVIDIA GPU: {torch.cuda.get_device_name(device)}")
    elif torch.backends.mps.is_available():
        # Apple Silicon (M1/M2/M3) MPS acceleration
        device = torch.device('mps')
        print(f"[Rank {rank}] Using Apple Silicon MPS (Metal Performance Shaders)")
    else:
        device = torch.device('cpu')
        print(f"[Rank {rank}] Using CPU")
    
    print(f"[Rank {rank}] Using device: {device}")
    
    # Load pretrained model
    if rank == 0:
        print(f"\nLoading pretrained tokenizer from: {config['pretrained_tokenizer_path']}")
    
    try:
        model = KronosTokenizer.from_pretrained(config['pretrained_tokenizer_path'])
        model.to(device)
        
        # Wrap with DDP only if we have multiple processes
        if world_size > 1:
            try:
                model = DDP(model, device_ids=[rank] if torch.cuda.is_available() else None)
                print(f"[Rank {rank}] Wrapped in DDP")
            except Exception as e:
                print(f"[Rank {rank}] DDP wrap failed: {e}, using model directly")
        
        if rank == 0:
            print("Model loaded successfully!\n")
            print("Starting training...\n")
        
        # Start training
        trained_model, results = train_model(model, device, config, save_dir, rank, world_size)
        
        if rank == 0:
            # Final summary
            print(f"\n{'='*70}")
            print("Training completed!")
            print(f"Best validation loss: {results['best_val_loss']:.4f}")
            print(f"Model saved to: {save_dir}/checkpoints/best_model")
            print(f"{'='*70}\n")
    
    except Exception as e:
        print(f"[Rank {rank}] Training failed: {e}")
        import traceback
        traceback.print_exc()
    
    finally:
        # Cleanup
        try:
            cleanup()
        except:
            pass


def main():
    """Main function using spawn instead of torchrun."""
    # Load configuration
    config_obj = Config()
    config = {
        'batch_size': config_obj.batch_size,
        'epochs': config_obj.epochs,
        'log_interval': config_obj.log_interval,
        'tokenizer_learning_rate': config_obj.tokenizer_learning_rate,
        'adam_beta1': config_obj.adam_beta1,
        'adam_beta2': config_obj.adam_beta2,
        'adam_weight_decay': config_obj.adam_weight_decay,
        'seed': config_obj.seed,
        'pretrained_tokenizer_path': config_obj.pretrained_tokenizer_path,
        'save_path': config_obj.save_path,
        'tokenizer_save_folder_name': config_obj.tokenizer_save_folder_name,
    }
    
    # Determine number of devices
    if torch.cuda.is_available():
        world_size = torch.cuda.device_count()
        print(f"Detected {world_size} CUDA GPU(s)")
    else:
        # For CPU-only systems, use multiprocessing (limited to avoid overhead)
        world_size = min(4, os.cpu_count() or 4)
        print(f"No CUDA available. Will use {world_size} CPU processes")
    
    # Setup save directory
    save_dir = os.path.join(config['save_path'], config['tokenizer_save_folder_name'])
    os.makedirs(os.path.join(save_dir, 'checkpoints'), exist_ok=True)
    print(f"Models will be saved to: {save_dir}")
    
    if world_size == 1:
        # Single device training (simpler, no need for spawn)
        print("\nRunning in single-process mode...")
        train_worker(0, 1, config, save_dir)
    else:
        # Multi-device training using spawn
        print(f"\nStarting distributed training with {world_size} processes using spawn...")
        print("This avoids torchrun's network initialization issues on macOS.\n")
        
        mp.spawn(
            train_worker,
            args=(world_size, config, save_dir),
            nprocs=world_size,
            join=True
        )


if __name__ == '__main__':
    main()
