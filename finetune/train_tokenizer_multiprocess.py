"""
Multi-GPU/CPU training script for Kronos Tokenizer finetuning using DataParallel.
This version uses PyTorch's DataParallel for multi-GPU training.
"""
import os
import sys
import time
...
def train_model(model, device, config, save_dir, rank=0, world_size=1):
    ...
    val_sampler = DistributedSampler(valid_dataset, num_replicas=world_size, rank=rank, shuffle=False) if world_size > 1 else None
...
def train_ddp(rank, world_size, config, save_dir):
...
def main():
    ...
    # Determine number of GPUs/devices
    if torch.cuda.is_available():
        world_size = torch.cuda.device_count()
        print(f"Detected {world_size} CUDA GPU(s)")
    else:
        # For CPU-only systems, use multiprocessing
        world_size = min(4, os.cpu_count() or 4)  # Limit to 4 processes for CPU
        print(f"No CUDA available. Using {world_size} CPU processes for parallel training")
    ...
    if world_size == 1:
        ...
        # Multi-GPU/CPU training
        print(f"\nStarting distributed training with {world_size} processes...\n")
        mp.spawn(
            train_ddp,
